...
class ExpenseController extends Controller
{
    ...
    {
    	       if (!isPermission('list', 'expense', $request->permission_list))
			return $this->jsonResponse('Permission Denied!', 403, "No Permission");
			
    	        $expense_no = $request->input('order_no','');
    ...
    { 
      ...
      //  if (!isPermission('edit', 'expense', $request->permission_list))
		//	return $this->jsonResponse('Permission Denied!', 403, "No Permission");
			
    	$userId = $request->input('user_id','');
    	$orderId = $request->input('expense_id','');
	$status = $request->input('status','');
	$cancel_reason = $request->input('cancel_reason','');

	
	$expense = Expense::with('expense_detail')->where('id',$expenseId)->first();
	...
	if(!empty($cancel_reason)){
		...
		// Partner Info
		 $partner = User::where('id',$expense->created_by)->first();
		
		 $config = $this->SystemConfig("Order Cancellation");
		 if(isset($config['description'] )){
		 
		 $detailList=[];
		 foreach($expense->expense_detail as $detail){
		           $product = Product::Join('product_variants as pv','pv.product_id','=','products.id')->where('pv.id',$detail->variant_id)->first();
			   $detail->product_name =  $product->name;
			   $detail->part_number =  $product->part_number;
			   $detailList[] = $detail; 
		   }
		  ...
		  $orderDetail = [
		   ...
		   $order_template = view('emails.expense-email', compact('orderDetail'))->render();
		  
		 $message = $config['description'] ?? "";
		 $subject = html_entity_decode($config['subject'] ?? "", ENT_QUOTES | ENT_HTML5);
		 $message = html_entity_decode($message, ENT_QUOTES | ENT_HTML5);
		 
		 $message = (str_replace('<Order ID>' ,"<a href='".env("SITE_URL").'/expenses/'.$expense->id."'>".$order->expense_no."</a>" , $message));
		 $message = (str_replace('<Link>' ,env("SITE_URL").'/expenses/'.$expense->id, $message));
		 $message = (str_replace('<Table Order>' ,$expense_template , $message));
		...
		// Partner Info
		 ...
		 if(isset($config['description'] )){
			 ...
			 $message = html_entity_decode($message, ENT_QUOTES | ENT_HTML5);
		         ...
		         $subject = (str_replace('<Order ID>' ,$order->order_no, $subject));
			 $message = (str_replace('<Remarks>' ,$order->remarks, $message));
			 $message = (str_replace('<Order Date>' ,$order->delivery_date, $message));
			 $message = (str_replace('<Order ID>' ,$order->order_no, $message));
			 $message = (str_replace('<Link>' ,env("SITE_URL").'/orders/'.$order->id, $message));
			//Email for Order
			 $mailData = [];
			 $mailData['name'] = 'Dear '.$partner->name;			   
			 $mailData['email'] = $partner->email;			  		
