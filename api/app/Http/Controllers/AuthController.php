...
class AuthController extends Controller
{
	...
	$user = UserToken::login($credentials);
        ...
        if($user) {
	    Cart::where('created_by',$user['user_id'])->delete();
...
//    }
	...
	$rules = [
	 ...
	 // Sent EMail     
	$config = $this->SystemConfig("Reset Password");
	...
	$message = (str_replace('<Reset Password>' , '<a href="'.$actionUrl.'" style="display: inline-block; padding: 0px 5px; font-size: 14px; color: #ffffff; background-color: #007bff; text-decoration: none; border-radius: 4px;">Reset Password</a>' , $message));			 
	$message = (str_replace('<Link>' , '<a href="'.env("SITE_URL").'">'.env("SITE_ADDRESS").'</a>', $message));
